...
void grabarLocal();
void mostrarLocales();
void calcularTotales(int ventas[][CANT_TRIMESTRE], int maxLocal);
void vaciarArchivo();
...
void mostrarLocales() {
    ...
    while (fscanf(file, "%d %d %d", &local.localNum, &local.trimestreNum, &local.importe) == 3) {
        ventas[local.localNum - 1][local.trimestreNum - 1] += local.importe;

        if (local.localNum > maxLocal) {
            maxLocal = local.localNum;
        }
    }
    ...
    calcularTotales(ventas, maxLocal);
}
